        ...
        if (tegevus.equals("encrypt")) {
            parool.setOnAction(e -> {
                if ((parool.getText().getBytes()).length == 16) {
                    ...
                    try {
                        Krüpteerija.tegevusFailiga(valitudFail, "krüpteerimine", parool.getText());
                        lavaParool.close();
                    } catch (IOException | InvalidKeyException ex) {
                        ex.printStackTrace();
                    }
                } else if (parool.getText().getBytes().length != 16) {
            ...
            parool.setOnAction(event -> {
                if ((parool.getText().getBytes()).length == 16) {
                    ...
                    veaSilt.setTextFill(Color.web("black"));
                    try {
                        Krüpteerija.tegevusFailiga(valitudFail, "dekrüpteerimine", parool.getText());
